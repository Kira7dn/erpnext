...
export function AccountingRecordActions({ resource, name }: Readonly<{ resource: AccountingResource; name: string }>) {
  ...
  async function run(action: string) { if (!window.confirm(`Run ${action} on ${name}?`)) return; setBusy(action); try { const response = await fetch(`/api/accounting/${resource}/${encodeURIComponent(name)}/${action}`, { method: "POST", headers: { Accept: "application/json" } }); if (!response.ok) throw new Error((await response.text()).slice(0, 300) || `HTTP ${response.status}`); router.refresh(); } catch (cause) { window.alert(cause instanceof Error ? cause.message : "Action failed"); } finally { setBusy(null); } }
  return <div className="flex flex-wrap gap-2">{available.map((action) => <Button key={action} disabled={busy !== null} variant={action === "cancel" || action === "unreconcile" ? "outline" : "default"} onClick={() => void run(action)}>{busy === action ? "Working..." : action.charAt(0).toUpperCase() + action.slice(1)}</Button>)}</div>;
}
