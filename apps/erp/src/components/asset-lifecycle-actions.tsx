...
export function AssetLifecycleActions({ resource, name, docstatus }: Readonly<{ resource: AssetResource; name: string; docstatus?: unknown }>) {
  ...
  return <div className="flex flex-wrap items-center gap-3 rounded-xl border bg-card p-5 shadow-sm"><div className="mr-auto"><h2 className="font-semibold">Document lifecycle</h2><p className="text-sm text-muted-foreground">Native ERPNext submit/cancel action.</p></div>{submitted ? <Button disabled={busy !== null} onClick={() => run("cancel")} variant="outline" type="button">{busy === "cancel" ? "Cancelling..." : "Cancel document"}</Button> : <Button disabled={busy !== null} onClick={() => run("submit")} type="button">{busy === "submit" ? "Submitting..." : "Submit document"}</Button>}{message ? <p className="basis-full text-sm text-muted-foreground">{message}</p> : null}</div>;
}
