...
export function AccountingRecordForm({ resource, fields, initial, name }: Readonly<{ resource: AccountingResource; fields: AccountingFormField[]; initial?: Record<string, unknown>; name?: string }>) {
  ...
  const mutation = useMutation({ mutationFn: ({ endpoint, payload }: { endpoint: string; payload: Record<string, unknown> }) => clientQuery<{ name?: string }>(endpoint, { method: name ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }), onSuccess: async (data) => { await queryClient.invalidateQueries({ queryKey: ["accounts", resource] }); const createdName = data?.name; router.push(createdName ? `/accounts/${resource}/${encodeURIComponent(createdName)}` : `/accounts/${resource}`); router.refresh(); } });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    for (const field of fields) {
      ...
      if (field.required && (value === "" || value === undefined)) { setError(`${field.name} is required`); return; }
      ...
      else if (field.type === "array") { try { payload[field.name] = JSON.parse(String(value)); } catch { setError(`${field.name} must contain valid JSON`); return; } }
      else payload[field.name] = value;
    }
    const endpoint = `/api/accounting/${resource}${name ? `/${encodeURIComponent(name)}` : ""}`;
    try { await mutation.mutateAsync({ endpoint, payload }); } catch (cause) { setError(cause instanceof Error ? cause.message : "Save failed"); }
  }
