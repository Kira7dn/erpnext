...
export async function UserSessionStatus() {
  const user = await getPortalUser();
  if (!user) {
    return <Link className="inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted" href="/api/auth/login"><span className="size-2 rounded-full bg-slate-400" />Chưa đăng nhập</Link>;
  }
