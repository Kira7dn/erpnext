...
const reports = [
  ...
  ["incorrectly-cleared", "Incorrectly Cleared Entries"],
] as const;
...
export function AccountingNativeReports() {
  ...
  const query = useQuery({
    ...
    enabled: false,
    queryFn: async () => {
      const params = new URLSearchParams({ report_key: report, filters });
      const payload = await clientQuery<Record<string, unknown>>(
        `/api/accounting/reports/report?${params}`,
      );
      ...
      const result: Row[] = Array.isArray(rawResult)
        ? (rawResult as Row[])
        : [];
      const columns = Array.isArray(data.columns)
        ? data.columns.map(
            (column: string | { fieldname?: string; label?: string }) =>
              typeof column === "string"
                ? column
                : (column.label ?? column.fieldname ?? "Column"),
          )
        : result[0]
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-[1fr_2fr_auto]">
        <select
        ...
        <input
          ...
          aria-label="Report filters JSON"
        />
        <button
          ...
          onClick={() => void run()}
        >
          {query.isFetching ? "Đang chạy…" : "Run report"}
        </button>
      </div>
      {query.error ? (
        <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">
          <p>
            {query.error instanceof Error
              ? query.error.message
              : "Không thể chạy report"}
          </p>
          {isAuthenticationError(query.error) ? <ClientAuthRecovery /> : null}
        </div>
      ) : null}
        ...
        {rows.length ? (
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-left">
              <tr>
                {columns.map((column) => (
                  <th className="whitespace-nowrap p-3" key={column}>
                    {column}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr className="border-t" key={String(row.name ?? index)}>
                  {columns.map((column) => (
                    <td className="whitespace-nowrap p-3" key={column}>
                      {String(row[column] ?? "—")}
                    </td>
                  ))}
                </tr>
          ...
          <p className="p-10 text-center text-sm text-muted-foreground">
            Chọn report và Run để tải dữ liệu live.
          </p>
        )}
      </div>
    </div>
