...
export function BankReconciliationActions() {
  const [action, setAction] = useState<(typeof actions)[number][0]>(
    actions[0][0],
  );
  ...
  const mutation = useMutation({
    mutationFn: (body: unknown) =>
      clientQuery(`/api/accounting/bank-reconciliation/actions/${action}`, {
        method: "POST",
  ...
  return (
    <section className="rounded-xl border bg-card p-4">
      <div>
        <h2 className="font-semibold">Reconciliation workflows</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Các thao tác native của Bank Reconciliation Tool. Payload dùng đúng
          field ERPNext yêu cầu.
        </p>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-[1fr_2fr_auto]">
        ...
        <textarea
          ...
          aria-label="ERPNext action payload"
        />
        <button
          ...
          onClick={() => void run()}
        >
          {mutation.isPending ? "Đang chạy…" : "Run"}
        </button>
      </div>
      {error ? (
        <div className="mt-3 rounded-md bg-destructive/10 p-3 text-sm text-destructive">
          <p>
            {error instanceof Error
              ? error.message
              : "Không thể thực hiện reconciliation action"}
          </p>
