...
function safeReturnTo(request: NextRequest): string {
  const requested = request.nextUrl.searchParams.get("return_to") ?? "/accounts";
  if (!requested.startsWith("/") || requested.startsWith("//") || requested.startsWith("/api/")) return "/accounts";
  const root = ["/assets", "/purchase", "/accounts"].find((value) => requested === value || requested.startsWith(`${value}/`));
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const nextResponse = NextResponse.redirect(new URL(safeReturnTo(request), portalAppBaseUrl(new URL(request.url).origin)));
