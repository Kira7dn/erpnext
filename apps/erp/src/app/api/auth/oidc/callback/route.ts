...
function safeReturnTo(request: NextRequest): string {
  const value = request.nextUrl.searchParams.get("return_to") ?? "/accounts/bank-accounts";
  if (!value.startsWith("/") || value.startsWith("//") || value.startsWith("/api/")) return "/accounts/bank-accounts";
  return value;
}

export async function GET(request: NextRequest): Promise<NextResponse> {
  const handoff = request.nextUrl.searchParams.get("handoff") ?? "";
  if (!handoff) return NextResponse.json({ error: "handoff_missing" }, { status: 400 });
  ...
  if (!response.ok) return NextResponse.json({ error: "handoff_invalid" }, { status: 401 });
  const payload = await response.json() as {
    user?: { id: string; email: string; displayName: string; avatarUrl: string | null };
    gateway_session?: string;
  };
  if (!payload.user || !payload.gateway_session) return NextResponse.json({ error: "handoff_invalid" }, { status: 401 });
  const session = await createErpSession(payload.user, payload.gateway_session);
  const next = NextResponse.redirect(new URL(safeReturnTo(request), request.url), 303);
