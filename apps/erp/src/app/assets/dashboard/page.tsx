...
export default async function AssetsDashboardPage() {
  ...
  return <AssetsShell><main className="mx-auto max-w-7xl space-y-8 p-5 md:p-8"><div><div className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Assets / Dashboard</div><h1 className="text-3xl font-bold tracking-tight">Asset dashboard</h1><p className="mt-2 text-muted-foreground">Các chỉ số được đọc trực tiếp từ ERPNext.</p></div>{error ? <Alert variant="destructive"><AlertCircle className="size-4" /><AlertTitle>Không lấy được dữ liệu</AlertTitle><AlertDescription>{error}</AlertDescription></Alert> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{cards.map(([label, value, Icon]) => <div className="rounded-xl border bg-card p-5 shadow-sm" key={label}><Icon className="size-5 text-emerald-700" /><p className="mt-5 text-sm text-muted-foreground">{label}</p><p className="mt-1 text-3xl font-bold">{value}</p></div>)}</div>}</main></AssetsShell>;
}
