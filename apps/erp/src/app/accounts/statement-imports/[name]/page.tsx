...
export default async function StatementImportDetailPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-5 md:p-8"><div className="flex items-center gap-3"><Link className="text-sm text-primary hover:underline" href="/accounts/statement-imports">← Statement imports</Link><h1 className="text-3xl font-bold">{decoded}</h1></div>{error ? <p className="rounded-md border border-destructive/40 bg-destructive/10 p-4 text-sm text-destructive">{error}</p> : <><div className="grid gap-5 lg:grid-cols-2"><Panel title="Import log" value={record} /><Panel title="Statement details" value={details} /></div><StatementDetailsEditor name={decoded} initialDetails={details} /></>}</main>;
}
