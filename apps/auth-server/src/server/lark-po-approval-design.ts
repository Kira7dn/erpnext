...
export type ApprovalDesignTableColumn = {
  key: string;
  id: string;
  label: string;
};
...
export const LARK_PO_APPROVAL_DESIGN = {
  i18nPrefix: "letron_po",
  // Lark Approval definition API requires a supported default resource locale.
  // The actual labels below are Vietnamese; this is not the text language.
  locale: "zh-CN",
  textLanguage: "vi-VN",
  approvalName: {
    key: "approval_name",
    label: "Phê duyệt mua hàng",
  },
  process: {
    approvalNode: {
      key: "node_approval",
      label: "Phê duyệt",
      approverUserId: "2619cbfe",
    },
    viewerType: "TENANT",
  },
  fields: [
...
export function larkI18nKey(suffix: string): string {
  return `@i18n@${LARK_PO_APPROVAL_DESIGN.i18nPrefix}_${suffix}`;
}
