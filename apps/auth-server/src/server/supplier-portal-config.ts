...
export function supplierPortalConfig(): SupplierPortalConfig {
  ...
  const config = {
    next_internal_base_url: process.env.LETRON_ERP_APP_BASE_URL || requiredString(source, "next_internal_base_url"),
    portal_public_base_url: process.env.LETRON_ERP_APP_BASE_URL || requiredString(source, "portal_public_base_url"),
