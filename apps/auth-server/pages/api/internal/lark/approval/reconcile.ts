...
import { reconcileApprovedPoDraft } from "../../../../../src/server/lark-purchase";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  if (getEnv().NODE_ENV === "production") {
    res.status(403).json({ error: "real_test_reconcile_disabled_in_production" });
    return;
  }
  if (!authorized(req)) {
  ...
  try {
    res.status(200).json(await reconcileApprovedPoDraft(instanceCode));
  } catch (error) {
    const message = error instanceof Error ? error.message : "lark_approval_reconcile_failed";
